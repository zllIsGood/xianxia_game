...
class ActivityWin extends BaseEuiView {
	...
	public static openCheck(actId: number, pageStyle: ActivityPageStyle): boolean {
		...
		if (!sum.length) {
			UserTips.ins().showTips(`|C:0xff0000&T:很遗憾,开服活动已经结束了`);
			return false;
		}

		for (let k in Activity.ins().activityData) {
			if (Activity.ins().activityData[k].pageStyle != pageStyle ||
	...
	private pageStyle: ActivityPageStyle;
	public open(actId: number, pageStyle: ActivityPageStyle): void {
        ...
        this.menuList.addEventListener(eui.ItemTapEvent.ITEM_TAP,this.onClickMenu,this);
		...
		this.observe(Activity.ins().postActivityIsSeek, this.refushRedPoint);
	...
	public close(...param: any[]): void {
		...
		this.menuList.removeEventListener(eui.ItemTapEvent.ITEM_TAP, this.onClickMenu, this);
	...
	private updateView() {
		...
		// let recharge: any = Recharge.ins().getRechargeData(1);
		// if (recharge && recharge.day >= 0) {
		// 	this._datas.push(Activity.ins().getbtnInfo(20 + ""));
		// }
	...
	private onClickMenu(e: eui.ItemTapEvent): void {
		...
		if (this.selectIndex != e.itemRenderer.itemIndex) {
		...
		this.selectIndex = e.itemRenderer.itemIndex;

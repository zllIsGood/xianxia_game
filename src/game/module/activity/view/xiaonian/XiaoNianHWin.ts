 ...
 * 红包入口
 */
class XiaoNianHWin extends BaseEuiView {
	/** 关闭按钮 */
	...
	public open(...param: any[]): void {
		...
		this.observe(GameApp.ins().postZeroInit, this.zeroUpdate);
	...
	private zeroUpdate() {
		let upView = false;
		let curTime = Math.floor(GameServer.serverTime / 1000);
		let actCount = 0;
		for (let k in Activity.ins().activityData) {
			if (Math.floor(DateUtils.formatMiniDateTime(Activity.ins().getActivityDataById(+k).startTime) / 1000) == curTime ||
				DateUtils.formatMiniDateTime(Activity.ins().getActivityDataById(+k).endTime) <= 0) {
				upView = true;
				break;
			}
			if (!actCount)
...
ViewManager.ins().reg(XiaoNianHWin, LayerManager.UI_Main);
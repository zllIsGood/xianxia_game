class OSATarget2Panel2 extends BaseView {
	...
	constructor() {
		...
		this.list.itemRenderer = OSATarget2ItemRender;
	}
	...
	public init() {
		let cfg = GlobalConfig.ActivityType2Config[this.activityID]
		let array = CommonUtils.objectToArray(cfg)
		this.list.dataProvider = new eui.ArrayCollection(array);

		let idx: number = this.getRule();
		this.updateData(idx);
		this.list.selectedIndex = idx - 1;

		let label: eui.Label;
		let config: ActivityType2Config;

		/*for (let i: number = 0; i < 4; i++) {
			label = this[`labName${i}`];
			if (label) {
				config = cfg[i + 1];
				label.text = config ? config.giftName : ``;
			}
		}*/

		this.mc = new MovieClip();
		this.addChild(this.mc);
		this.mc.x = 120, this.mc.y = 330;
		this.mc.playFile(`${RES_DIR_EFF}artifacteff2`, -1);

	}

	public open(...param: any[]): void {
		egret.Tween.get(this.showbg, { loop: true }).to({ y: this.showbg.y + 20 }, 1500).to({ y: this.showbg.y }, 1500);
	...
	public close(...param: any[]): void {
		...
		this.removeObserve();
		// egret.Tween.removeTweens(this.showbg);
		// //这里矫正位置防止缓动造成的偏差
		// this.showbg.y = -26;
	...
	onItemTap(e: eui.ItemTapEvent){
		let index = e.itemRenderer.itemIndex + 1

class OSATarget9Panel1 extends BaseView {
    ...
    private help: eui.Button;//帮助说明
    private static helpIndex: number = 40;//帮助说明index
    ...
    public close(...param: any[]): void {
        ...
        this.removeObserve();
        this.removeTouchEvent(this.help, this.onClick);
        for( let i = 0;i < 3;i++ ){
    ...
    private listRefush(): void {
        ...
        if( data ){
            ...
            for( let i = 0; i < data.noticeArr.length;i++ ){
                let notice = {activityID:this.activityID,name:data.noticeArr[i].name,index:data.noticeArr[i].index};
                arr.push(notice);
            }
    ...
    private onClick(e: egret.TouchEvent): void {
        switch (e.currentTarget) {
            case this.help:
                //显示帮助面板
                ViewManager.ins().open(ZsBossRuleSpeak,40);
                break;
        }
    }
}

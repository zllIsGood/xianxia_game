 ...
 *
 * @author AlexLam 
 * 临时弄的，2018 2 12发版，2018 2 11就要完成，后续完善
 *
 */
class ActivityFestivalWin extends BaseEuiView {
	/** 关闭按钮 */
	...
	public static openCheck(...param: any[]): boolean {
		...
		UserTips.ins().showTips(`|C:0xff0000&T:很遗憾,节日活动已经结束了`);
		return false;
	}
	...
	public open(...param: any[]): void {
		...
		this.title.source = "activity_word3"
	}
	...
	private updateView() {
		...
		for (let k in Activity.ins().activityData) {
			if (Activity.ins().activityData[k].pageStyle != ActivityPageStyle.FESTIVAL) {
				continue;//此界面不处理非节日活动外的活动
			}
			if (Activity.ins().getActivityDataById(+k).isOpenActivity()) {
				let data: ActivityBtnConfig = Activity.ins().getbtnInfo(k);
				//过滤嵌套活动
				if (!ErrorLog.Assert(data, "ActivityFestivalWin  data   " + k) &&  data.activityType != ActivityType.Nesting) {
...
ViewManager.ins().reg(ActivityFestivalWin, LayerManager.UI_Main);
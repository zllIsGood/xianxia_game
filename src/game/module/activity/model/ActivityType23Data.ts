class ActivityType23Data extends ActivityBaseData {
	...
	public update(bytes: GameByteArray): void {
		...
		for (let i = 0; i < this.count; i++) {
			...
			data.rechargeCount = bytes.readShort();
			data.rewardCount = bytes.readShort();
			this.activityData[data.index] = data;
		}
	}
	...
	public canReward(): boolean {
		...
		for (let i in this.activityData) {
			data = this.activityData[i];
			if (data.rechargeCount > data.rewardCount) {
				can = true;
				break;
			}
		}

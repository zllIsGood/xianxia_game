 ...
 *
 */
class StarList extends BaseView {
	...
	public constructor(listLength: number = 10, starNum: number = 0, spacing: number = 45, isShowNum: number = 1) {
		super();
		...
		for (let i: number = 0; i < this._statListLength; i++) {
			if (i <= this._starNum - 1) {
				...
				if (show == 1 && i == this._starNum - 1) {
					this.mc.x = this.list[i].x + 13;
					this.mc.y = this.list[i].y + 15;
					if (!this.mc.parent) {
	...
	public starSize(x:number,y:number):void{
		this.mc.scaleX = x;
		this.mc.scaleY = x;
	}

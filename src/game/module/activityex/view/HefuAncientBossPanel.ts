 ...
 * 合服远古boss
 */
class HefuAncientBossPanel extends BaseView {
	...
	private initView():void{
		...
		for( i = this.openTime.length-1;i >= 0;i-- ){
			...
			let idx = i-1;
			if( idx >= 0 ){
				let cityBoss: CityBossConfig = this.getBossConfig(i);
		...
		if(!b){
			if(GameServer.serverTime < firstNum){
				//未开启
				startIndex = -1;
				// GlobalConfig.CityBossConfig[1].bossId;
		...
		let cbcfg:CityBossConfig = GlobalConfig.CityBossConfig[cbid];
		if( cbcfg ){
	...
	private selectCurBoss(bossId:number,startIndex:number,b:boolean=false):void{
    	...
    	if( startIndex >= 0 ){
    		let bid = GlobalConfig.CityBossConfig[startIndex+1].bossId;
		...
		if(startIndex  >= 0){
			if(!b){
				...
				let cityBoss: CityBossConfig = this.getBossConfig(startIndex+1);
				let killBossId: number = cityBoss.killBossId;
	...
	public static getCityBossConfig(bossId: number): CityBossConfig {
	...
	//根据序列号
	private getBossConfig(index:number):CityBossConfig{

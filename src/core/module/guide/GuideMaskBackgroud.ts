...
class GuideMaskBackgroud extends egret.Sprite {
	...
	public draw(deductRec: egret.Rectangle): void {
		...
		let minX: number = Math.max(deductRec.x, 0);
		let minY: number = Math.max(deductRec.y, 0);
		let maxX: number = Math.min(deductRec.x + deductRec.width, this._stageWidth);
		let maxY: number = Math.min(deductRec.y + deductRec.height, this._stageHeight);
